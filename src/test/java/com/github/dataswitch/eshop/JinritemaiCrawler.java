...
//                System.out.println("正在爬取第 " + currentPage + " 页，共 " + totalPages + " 页 jsonResponse:"+jsonResponse);
    ...
    private static String fetchApiData(int page) throws IOException {
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            Map responseMap = parseJsonForMap(responseBody);
	...
	private static Map parseJsonForMap(String responseBody) throws JsonProcessingException, JsonMappingException {
		try {
			Map responseMap = objectMapper.readValue(responseBody, Map.class);
			return responseMap;
		}catch(Exception e) {
			throw new RuntimeException("parse json error:"+responseBody,e);
		}
	}
    ...
    private static String buildRequestBody(int currentPage) {
    	SearchRequest r = SearchRequest.createWithDefaultValues();
    	r.getPage().setCurrent(currentPage);
    	r.getPage().setPage_size(pageSize);
    	...
    	try {
			String newJson = objectMapper.writeValueAsString(r);
			System.out.println("buildRequestBody() oldJson:"+oldJson);
			System.out.println("buildRequestBody() newJson:"+newJson);
			return newJson;
